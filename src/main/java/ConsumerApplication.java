...
public class ConsumerApplication {
    private static final Logger logger = Logger.getLogger(ConsumerApplication.class.getName());
    ...
    private static void start() throws IOException {
        ...
        String port = Optional.ofNullable(System.getenv("PORT")).orElse("8082");

        // Inicia o servidor HTTP
        URI baseUri = UriBuilder.fromUri("http://" + hostname + "/").port(Integer.parseInt(port)).build();
        ResourceConfig config = new ResourceConfig(ConsumerResource.class);
